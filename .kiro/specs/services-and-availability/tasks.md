...
- [ ] 1. Připravit testovací infrastrukturu a typy pro `Slot_Calculator`
  - [ ] 1.1 Vytvořit doménové typy v `src/lib/slots/types.ts`
  ...
  - [ ] 1.2 Nainstalovat a nakonfigurovat Vitest + fast-check
...
- [ ] 2. Implementovat `Slot_Calculator` jako čistou funkci
  - [ ] 2.1 Implementovat helper utility v `src/lib/slots/time.ts`
  ...
  - [ ] 2.2 Implementovat `calculateSlots` v `src/lib/slots/calculator.ts`
  ...
  - [ ]* 2.3 Napsat unit testy pro `calculateSlots` v `src/lib/slots/calculator.test.ts`
    - Zavřený den → `[]`
    - `opensAt >= closesAt` → `[]`
  ...
  - [ ]* 2.4 Property test 1 — Adjacency v `src/lib/slots/calculator.property.test.ts`
  ...
  - [ ]* 2.5 Property test 2 — Overlap symmetry
  ...
  - [ ]* 2.6 Property test 3 — Tolerance and grid alignment
  ...
  - [ ]* 2.7 Property test 4 — Parallel slots invariant
  ...
  - [ ]* 2.10 Property test 7 — Ascending order
    - **Property 7: Výsledek je seřazený vzestupně**
    - Pro libovolný validní vstup ověř `result[i] < result[i+1]` (žádné duplicity)
    - **Validates: Requirements 9.8**
    - _Properties: 7_

- [ ] 3. Checkpoint — `Slot_Calculator` hotový
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Implementovat `Public_Page_Revalidator`
  - [ ] 4.1 Vytvořit utilitu `src/lib/revalidate.ts`
  ...
  - [ ]* 4.2 Unit test pro `revalidatePublicPage`
    - Mock `revalidatePath`; ověř volání s `/{slug}`
    - Ověř, že selhání `revalidatePath` neházi výjimku
    - _Requirements: 10.4_

- [ ] 5. Implementovat `Services_Manager` server actions
  - [ ] 5.1 Vytvořit Zod schéma pro službu v `src/lib/services/schema.ts`
  ...
  - [ ] 5.2 Implementovat `listServices` server action v `src/app/dashboard/services/actions.ts`
    - Načíst služby pro `business_id` přihlášeného uživatele, řadit `created_at ASC`
    - _Requirements: 1.1, 11.1_

  - [ ] 5.3 Implementovat `createService` server action
  ...
  - [ ] 5.4 Implementovat `updateService` server action
    - Validovat vstup, ověřit `business_id`, UPDATE, log, revalidace
    - _Requirements: 1.3, 2.10, 11.1, 11.5, 12.2_

  - [ ] 5.5 Implementovat `getReservationCount` server action
  ...
  - [ ] 5.6 Implementovat `deleteService` server action
  ...
  - [ ]* 5.7 Unit testy pro Zod schéma služby
    - Pokrýt všechny validační větve z R2 (každá hláška má test)
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8, 2.9_

- [ ] 6. Implementovat UI služeb
  - [ ] 6.1 Vytvořit server komponentu `src/app/dashboard/services/page.tsx`
  ...
  - [ ] 6.2 Vytvořit klientskou komponentu `src/app/dashboard/services/ServiceForm.tsx`
  ...
  - [ ] 6.3 Vytvořit klientskou komponentu `src/app/dashboard/services/DeleteServiceDialog.tsx`
...
- [ ] 7. Implementovat `OpeningHours_Manager`
  - [ ] 7.1 Vytvořit Zod schéma pro otevírací dobu v `src/lib/opening-hours/schema.ts`
  ...
  - [ ] 7.2 Implementovat server actions v `src/app/dashboard/opening-hours/actions.ts`
  ...
  - [ ]* 7.3 Unit testy pro Zod schéma otevírací doby
    - `opens_at >= closes_at` → chyba R4.4
    - Všechny dny zavřené → chyba R4.5
    - _Requirements: 4.4, 4.5_

- [ ] 8. Implementovat UI otevírací doby
  - [ ] 8.1 Vytvořit server komponentu `src/app/dashboard/opening-hours/page.tsx`
    - Načíst týden přes `listOpeningHours`
    - Vykreslit `OpeningHoursForm`
    - _Requirements: 4.1_

  - [ ] 8.2 Vytvořit klientskou komponentu `src/app/dashboard/opening-hours/OpeningHoursForm.tsx`
...
- [ ] 9. Implementovat `Settings_Manager`
  - [ ] 9.1 Implementovat server actions v `src/app/dashboard/settings/actions.ts`
...
- [ ] 10. Implementovat UI nastavení
  - [ ] 10.1 Vytvořit server komponentu `src/app/dashboard/settings/page.tsx`
    - Načíst nastavení přes `getSettings`
    - Vykreslit `SettingsForm`
    - _Requirements: 5.1, 6.1_

  - [ ] 10.2 Vytvořit klientskou komponentu `src/app/dashboard/settings/SettingsForm.tsx`
...
- [ ] 11. Checkpoint — UI hotové
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Migrace — ověřit `ON DELETE CASCADE` na `reservations.service_id`
  - [ ] 12.1 Zkontrolovat existující migraci `0004_init_reservations_clients.sql`
...
- [ ] 13. Integrační testy
  - [ ]* 13.1 Test RLS izolace v `tests/integration/services-rls.test.ts`
  ...
  - [ ]* 13.2 Test cascade delete v `tests/integration/services-cascade.test.ts`
  ...
  - [ ]* 13.3 Test revalidačního volání v `tests/integration/revalidation.test.ts`
    - Mock `revalidatePath`
...
- [ ] 14. Final checkpoint — všechny testy běží
  - Ensure all tests pass, ask the user if questions arise.
