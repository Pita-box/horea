...
- [x] 1. Datová vrstva a migrace
  - [x] 1.1 Migrace tabulky `subscriptions`
  ...
  - [x] 1.2 Migrace tabulky `invoice_counter`
  ...
  - [x] 1.3 Migrace tabulky `payments`
    - Přidat sloupec `invoice_number` a unique constraint na `variable_symbol` i `invoice_number` napříč celou tabulkou
    - _Requirements: 5.2, 7.4_

- [x] 2. Stavový automat a doménové výpočty (čisté funkce)
  - [x] 2.1 Implementovat `computeState(first_failed_charge_at, now)`
  ...
  - [x]* 2.2 Property test pro stavový automat
  ...
  - [x] 2.3 Implementovat aplikaci přechodů a doprovodné efekty
  ...
  - [x] 2.4 Implementovat prodloužení období `extendPeriod`
  ...
  - [x]* 2.5 Property test pro prodloužení období
    - **Property 6: Prodloužení období o přesně jeden měsíc**
    - _Properties: 6_
    - _Requirements: 1.5, 2.3, 5.5_

- [x] 3. Generátor variabilního symbolu
  - [x] 3.1 Implementovat `generateVariableSymbol`
  ...
  - [x]* 3.2 Property test pro variabilní symbol
...
- [x] 4. SPAYD QR generátor
  - [x] 4.1 Implementovat `encodeSpayd` a generování QR
  ...
  - [x]* 4.2 Property test pro SPAYD round-trip
    - **Property 4: SPAYD round-trip**
    - Generátor varíruje IBAN, částku, VS; ověří, že encode→decode zachová IBAN, částku, měnu a VS
    - _Properties: 4_
    - _Requirements: 4.4, 4.7_

- [x] 5. Faktura_Generator
  - [x] 5.1 Implementovat atomické přidělení čísla faktury
  ...
  - [x]* 5.2 Property test pro číslování faktur
  ...
  - [x] 5.3 Implementovat generování PDF a uložení faktury
...
- [x] 6. Checkpoint — průběžná validace jádra
  - Ensure all tests pass, ask the user if questions arise.

- [x] 7. Aplikace kupónu
  - [x] 7.1 Implementovat validaci kupónu
  ...
  - [x]* 7.2 Unit test validace kupónu
    - Neexistující / expirovaný / vyčerpaný kupón → platba nezahájena + česká hláška
    - _Requirements: 9.1, 9.2_

  - [x] 7.3 Implementovat aplikaci slevy a aktivačních kupónů
  ...
  - [x]* 7.4 Property test pro slevu kupónu
...
- [x] 8. Checkout a první platba
  - [x] 8.1 Implementovat mapování tarif → částka a vytvoření Payment
  ...
  - [x]* 8.2 Unit test mapování tarif → částka
    - Ověřit částky 199/299/599 a vytvoření Payment `pending`/`auto_charge`
    - _Requirements: 1.1_

  - [x] 8.3 Implementovat checkout server action `/api/checkout`
...
- [x] 9. Webhook handler
  ...
  - [x]* 9.4 Property test idempotence webhooku
...
- [x] 10. Billing_Engine
  - [x] 10.1 Implementovat založení recurring schedule
  ...
  - [x] 10.2 Implementovat měsíční charge
  ...
  - [x]* 10.3 Unit test billing
    - Payment `pending` vzniká před charge; selhání iniciace ponechá `active` + notifikace admina
    - _Requirements: 2.4, 2.5_

- [x] 11. Checkpoint — platební jádro
  - Ensure all tests pass, ask the user if questions arise.

- [x] 12. Cron úlohy a konfigurace
  - [x] 12.1 Implementovat Billing_Cron `/api/cron/billing`
  ...
  - [x] 12.2 Implementovat Cleanup_Cron `/api/cron/cleanup`
  ...
  - [x]* 12.3 Property test úplnosti mazání dat
  ...
  - [x] 12.4 Implementovat Warning_Cron `/api/cron/warnings`
  ...
  - [x]* 12.5 Unit test warning predikátů
  ...
  - [x] 12.7 Konfigurace Vercel Cron ve `vercel.json`
    - Deklarovat denní cron triggery pro `/api/cron/billing`, `/api/cron/warnings`, `/api/cron/cleanup`
    - _Requirements: 10.1, 10.4, 10.5_

- [x] 13. Změna tarifu
  - [x] 13.1 Implementovat žádost o změnu a její zrušení
...
- [x] 14. Zrušení a obnova automatické obnovy
  ...
  - [x]* 14.2 Unit test přepínání auto-obnovy
    - Zrušení i opětovné zapnutí auto-obnovy
    - _Requirements: 11.1, 11.4_

- [x] 15. České e-mailové šablony
  - [x] 15.1 Implementovat e-mailové šablony
...
- [x] 16. Stránka předplatného a napojení UI
  - [x] 16.1 Implementovat `/dashboard/subscription`
