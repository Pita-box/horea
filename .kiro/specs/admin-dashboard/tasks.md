...
- [x] 1. Datová vrstva a migrace
  - [x] 1.1 Migrace tabulky `audit_log`
  ...
  - [x] 1.2 Vynutit append-only nad `audit_log` na úrovni DB
  ...
  - [x] 1.3 Migrace sloupce `coupons.is_active`
...
- [x] 2. Řízení přístupu (Access_Guard + RLS admin override)
  - [x] 2.1 Implementovat Access_Guard middleware pro `/admin/*`
  ...
  - [x] 2.2 Ověřit / rozšířit RLS admin override policies
  ...
  - [x]* 2.3 Property test pro řízení přístupu
...
- [x] 3. AuditLogger (auditní jádro)
  - [x] 3.1 Implementovat AuditLogger
  ...
  - [x]* 3.2 Property test úplnosti auditní stopy
  ...
  - [x]* 3.3 Property test append-only auditní stopy
  ...
  - [x]* 3.4 Unit test best-effort zachycení before/after
...
- [x] 4. Checkpoint — datová vrstva, přístup a audit
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Přehled a statistiky
  - [x] 5.1 Implementovat StatsAggregator
  ...
  - [x]* 5.4 Unit test metrik přehledu
...
- [x] 6. Prohlížení auditní stopy
  - [x] 6.1 Implementovat AuditLogViewer
...
- [x] 8. Detail podniku
  ...
  - [x] 8.2 Implementovat stránku `/admin/businesses/[id]`
  ...
  - [x]* 8.3 Unit test detailu podniku
    - Profil + vlastník, stav/tarif/`current_period_end`, řádek historie plateb, počet rezervací
    - _Requirements: 4.1, 4.2, 4.3, 4.4_

- [x] 9. Checkpoint — čtecí plochy dashboardu
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Override předplatného a pozastavení podniku
  - [x] 10.1 Implementovat override předplatného + audit
...
- [x] 11. Free trial a comp účet
  - [x] 11.1 Implementovat udělení Free_Trial a Comp_Ucet (atomicky) + audit
  ...
  - [x]* 11.2 Property test atomicity Free_Trial a Comp_Ucet
  ...
  - [x]* 11.3 Unit test cílových hodnot Free_Trial / Comp_Ucet
    - Free_Trial (active + is_published + period_end, bez platby) a Comp_Ucet (active + is_published, bez platby a bez schedule)
    - _Requirements: 5.3, 5.4_

- [x] 12. Vynucené smazání podniku
  - [x] 12.1 Implementovat vynucené smazání + audit
  ...
  - [x]* 12.2 Property test úplnosti vynuceného smazání
...
- [x] 13. Checkpoint — admin akce nad podnikem
  - Ensure all tests pass, ask the user if questions arise.

- [x] 14. Správa kupónů (CRUD)
  - [x] 14.1 Implementovat CouponManager CRUD + audit
  ...
  - [x] 14.2 Implementovat stránku `/admin/coupons`
  ...
  - [x]* 14.3 Property test validace kupónu
  ...
  - [x]* 14.4 Unit test CRUD kupónů
    - Persistence atributů při vytvoření a úpravě, deaktivace, smazání, obsah seznamu s počtem použití
    - _Requirements: 7.1, 7.4, 7.5, 7.6, 7.7_

- [x] 15. Ruční párování plateb
  - [x] 15.1 Implementovat PaymentMatcher + audit
...
- [x] 16. Opětovné odeslání faktury
  ...
  - [x]* 16.2 Unit test opětovného odeslání faktury
    - Ověření existence paid faktury a výběr nejnovější; absence faktury → česká hláška
    - _Requirements: 11.1, 11.2_

- [x] 17. Napojení admin akcí do detailu podniku
  - [x] 17.1 Napojit akční tlačítka do `/admin/businesses/[id]`
...
- [x] 18. Checkpoint — všechny admin akce napojeny
  - Ensure all tests pass, ask the user if questions arise.

- [x] 19. Integrační testy (lokální Supabase / Docker)
  - [x]* 19.1 Integrační test RLS admin override
    - Admin přečte řádky více podniků, ne-admin je nepřečte
    - _Requirements: 1.4, 1.5_

  - [x]* 19.2 Integrační test append-only auditu na úrovni DB
    - Pokus o `UPDATE`/`DELETE` nad `audit_log` je odmítnut i se service role
    - _Requirements: 9.5_
    - _Properties: 3_

  - [x]* 19.3 Integrační test spárování platby → efekt prodloužení
    - Nastavení `paid` vyvolá efekt definovaný v `subscription-payments`; korektnost prodloužení vlastní onen spec
    - _Requirements: 8.3_

  - [x]* 19.4 Integrační test úplnosti vynuceného smazání
    - Proti reálné DB ověřit úplnost mazání tenant dat a zachování historie `subscriptions`/`payments`
    - _Requirements: 6.2, 6.3_
    - _Properties: 7_

- [x] 20. End-to-end testy (Playwright, happy path)
  - [x]* 20.1 E2E test CRUD kupónu
