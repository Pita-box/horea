-- ---------------------------------------------------------------------------
-- 0050 — Multi-service rezervace: (re)vytvoření RPC funkcí + reload schema cache
-- ---------------------------------------------------------------------------
-- OPRAVA NASAZENÍ: migrace 0049 vytvořila tabulku reservation_services a backfill,
-- ale tři RPC funkce (create_reservation_multi, create_manual_reservation_multi,
-- edit_reservation_multi) NEBYLY na sdílené DB k dispozici přes PostgREST
-- (PGRST202 „Could not find the function ... in the schema cache"), takže odeslání
-- multi-service rezervace končilo serverovou chybou 500.
--
-- Tato migrace je IDEMPOTENTNÍ: definice funkcí jsou převzaty 1:1 z 0049 a používají
-- `create or replace function`, takže je bezpečné ji spustit bez ohledu na to, zda
-- funkce v DB již existují, nebo ne. Na konci explicitně přenačte PostgREST schema
-- cache (`notify pgrst, 'reload schema'`), aby byly funkce ihned volatelné přes REST.
--
-- Schéma se jinak nemění; tabulka i backfill z 0049 zůstávají beze změny.
-- ---------------------------------------------------------------------------
...
-- Přenačtení PostgREST schema cache, aby byly nové funkce ihned volatelné přes REST.
