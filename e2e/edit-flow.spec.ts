 ...
 *
 * Tok: detail upravitelné rezervace → dialog „Upravit" → změna času na dostupný
 * slot → Uložit → potvrzení (dialog se zavře bez chyby). Je-li nakonfigurován
 * inbox (`E2E_RESEND_INBOX_URL`), ověří se navíc `Reservation_Modified_Email`.
 *
 ...
 */
test('majitel upraví čas rezervace na dostupný slot a uloží změnu', async ({ page }) => {
  test.skip(
    !hasOwnerCreds,
  ...
  // (R9.4). Z nabídky pak vybereme reálně dostupný slot a uložíme.
