...
type ReservationActionsProps = {
  ...
  /** Aktuální služba rezervace — předvyplnění editačního formuláře. */
...
export function ReservationActions({
  ...
  const [editServiceId, setEditServiceId] = useState(currentServiceId ?? services[0]?.id ?? '');
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {status === 'pending' || status === 'approved' ? (
          <Button
            ...
            variant="ghost"
            onClick={() => {
              setEditServiceId(currentServiceId ?? services[0]?.id ?? '');
      ...
      {dialog === 'edit' ? (
        <Modal titleId="edit-title" title="Upravit rezervaci" onClose={closeDialog}>
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium" htmlFor="edit-service">
              Služba
            </label>
            <select
              id="edit-service"
